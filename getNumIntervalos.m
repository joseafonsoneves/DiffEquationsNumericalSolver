...
while a == 0
    if modo == 'B'
        numIntervalos = str2double(input('Defina o número de intervalos inicial: ', 's'));
    else
        numIntervalos = str2double(input('\nDefina o número de intervalos: ', 's'));
    end
    
    if isnan(numIntervalos) || round(numIntervalos) ~= numIntervalos || numIntervalos < 1
