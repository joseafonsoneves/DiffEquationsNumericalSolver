...
while sair == 0
    ...
    switch opcao
        case 'A'   
            ...
            solveAviao('-0.02 * y^2 - 0.9', [0 (300 / 3.6)], 20, ...
                       numIntervalos, metodo);
        case 'B'
            fprintf('\nDefina os parâmetros a e b da equação do');
            fprintf(' sistema de travagem, que tem a forma \n');
            fprintf('F(t,y) = -a*y^2 - b, com a>0 e b>0.\n');
            a = input('a = ');
            b = input('b = ');
            strFuncao = sprintf('-%d * y^2 - %d', a, b);

            fprintf('\nDefina o instante de tempo inicial (s) e a ');
            fprintf('velocidade inicial (km/h), respetivamente a \n');
            fprintf('abcissa e aordenada do ponto inicial.\n');
            pontoInicial = getPontoInicial('');
            pontoInicial(2) = pontoInicial(2) / 3.6; % Conversão para m/s

            fprintf('\nDefina o intervalo de tempo onde pretende ');
            fprintf('saber se o avião se imobiliza.\n');
            fprintf('Sugestão: Intervalos de tempo muito grandes dão');
            fprintf(' origem a erros de aproximação.\n');
            extremoDireito = getExtremoDireito(pontoInicial(1));
            
            metodo = getMetodo();
            numIntervalos = getNumIntervalos('');
            
            solveAviao(strFuncao, pontoInicial, extremoDireito, ...
                       numIntervalos, metodo);
        case 'S'
            sair = 1;
        otherwise
...
function solveAviao(strFuncao, pontoInicial, extremoDireito, ...
                    numIntervalos, metodo)
    funcao = str2func(['@(t,y)', strFuncao]);
    
    %calcula a solução através de um dos métodos
    [x, y] = calculaSolucaoNum(funcao, pontoInicial, extremoDireito, ...
                               numIntervalos, metodo);

    for i = 1 : numIntervalos
        %Encontra a 1ª abcissa do ponto cuja ordenada é 0
        %Apenas para sistemas de travagem
        if (y(i) > 0 && y(i+1) <= 0)
            zero = x(i+1) - x(1); %Intervalo de tempo
            fprintf(['---------------------------------------------', ... 
                    '----------|\n']);
            fprintf(['O avião imobiliza-se ao fim de %f', ...
                    'segundos.\n'], zero);
            fprintf(['---------------------------------------------', ... 
                    '----------|\n']);
        ...
        if i == numIntervalos
            plot(x, y, '-r+', [0 extremoDireito], [0 0], 'g');
            alteraGraf(metodo, strFuncao, 'A');
            fprintf(['O avião não se imobiliza no intervalo', ...
                    'considerado.\n']); 
        end  
    end
